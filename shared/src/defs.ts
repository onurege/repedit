...
export type ProductId = 'milk' | 'beans' | 'coffee';
...
export type BusinessType = 'farm' | 'coffee_shop';
...
// Demand multiplier from coffee price (1.0 at base price $30).
export function priceDemandMultiplier(price: number): number {
  const m = 1.6 - 0.6 * (price / PRODUCTS.coffee.basePrice);
